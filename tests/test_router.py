...
class TestRouter:
    ...
    # post vertex
    data_for_post_vertices = [(201, "/v1/graph/vertices", {"label": "individual", "name": "Z"})]

    @pytest.mark.asyncio
    @pytest.mark.parametrize("expected, endpoint, params", data_for_post_vertices)
    async def test_post_vertices(self, client, gremlin_service, expected, endpoint, params):
    ...
    async def test_post_edges(self, client, gremlin_service, expected, endpoint, params):
        response = await client.post(endpoint, json=params)
        assert expected == response.status_code
